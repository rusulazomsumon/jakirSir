  ...
  return (
    <section className="pt-6" id="mockTestSection">
      <div className="rounded-[16px] border border-border bg-white p-4 shadow-card">
        {!started ? (
          <div>
            <h3 className="font-semibold mb-2">📝 মক টেস্ট</h3>
            <div className="flex gap-2 mb-3">
              <select value={course} onChange={(e) => setCourse(e.target.value)} className="flex-1 rounded-md border border-border p-2">
                <option>Bank Math</option>
                <option>GK</option>
                <option>English</option>
              </select>
              <select value={subject} onChange={(e) => setSubject(e.target.value)} className="flex-1 rounded-md border border-border p-2">
                <option>Quant</option>
          ...
          <div>
            ...
            <motion.div key={questions[current].id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="mb-3">
              ...
              <div className="grid grid-cols-1 gap-2">
                {questions[current].choices.map((c, idx) => {
                  ...
                  return (
                    <button key={idx} onClick={() => select(questions[current].id, idx)} className={`w-full text-left rounded-md p-3 border ${selected ? 'border-primary bg-primary/10' : 'border-border'} transition`}>{String.fromCharCode(65+idx)}. {c}</button>
                  )
                })}
              </div>
            </motion.div>
            ...
            <div className="flex items-center gap-2 mt-4">
              <button disabled={current === 0} onClick={() => setCurrent((c) => Math.max(0, c-1))} className="inline-flex h-10 items-center justify-center rounded-md border border-border px-3">Prev</button>
              <button disabled={current === questions.length-1} onClick={() => setCurrent((c) => Math.min(questions.length-1, c+1))} className="inline-flex h-10 items-center justify-center rounded-md border border-border px-3">Next</button>
            ...
            <Modal open={openResult} onClose={() => setOpenResult(false)}>
              <div>
                ...
                <div className="mt-4 flex gap-2">
                  <button onClick={() => { setOpenResult(false); setStarted(false); }} className="inline-flex h-[44px] items-center justify-center rounded-md border border-border px-4">Close</button>
                  <button onClick={() => { setOpenResult(false); start(); }} className="inline-flex h-[44px] items-center justify-center rounded-md bg-primary px-4 text-white">Retry</button>
                </div>
              </div>
            </Modal>
          </div>
